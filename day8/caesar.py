...
def encrypt(plain_text: str, key: int):
    ...
    for letter in plain_text:
        if letter.isalpha():    # アルファベットの場合のみデコード処理を行う
            # 大文字の場合
            if letter.isupper():
                cipher_text += chr(ord('A') + (ord(letter) - ord('A') + key) % 26)     # シフト後の文字を計算
            # 小文字の場合
            else:
                cipher_text += chr(ord('a') + (ord(letter) - ord('a') + key) % 26)
        # アルファベット以外はそのまま追加
        else:
            cipher_text += letter
...
def decrypt(cipher_text: str, key: int):
    ...
    for letter in cipher_text:
        if letter.isalpha():
            if letter.isupper():
                plain_text += chr(ord('Z') - (ord('Z') - ord(letter) + key) % 26)
            else:
                plain_text += chr(ord('z') - (ord('z') - ord(letter) + key) % 26)
        else:
            plain_text += letter
